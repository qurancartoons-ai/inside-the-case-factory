...
Script generation uses a bounded writer → critic → rewriter pipeline: one initial low-cost script attempt and at most two targeted revisions. A deterministic local critic converts concrete validator evidence into a structured repair plan with exact affected passages. The rewriter receives the existing script, approved claims, story architecture, and that plan; it must preserve unaffected text and may not introduce facts. Every candidate is validated immediately, and generation stops as soon as the first fully valid candidate is safely promoted. Confirmation and per-project spending checks still run before every provider call.
